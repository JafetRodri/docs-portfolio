...
These are published knowledge base articles — policies, SOPs, and standards — rendering **inline** below exactly as they'd look in a live system. Scroll through them, or jump to one:
...
!!! tip "How these were built — and why they look like this"
    You may notice things a normal knowledge base doesn't usually have: colored callouts, TLP classification pills, workflow grids, per-step metadata blocks, an in-page table of contents. That's deliberate.

    **I built a document generation tool to produce them.** It takes an organization's brand palette and a document type, and outputs a fully structured, on-brand HTML article with the governance scaffolding already in place — ISO-aligned document control blocks, TLP classification, version history, confidentiality notices, and a consistent component library across every document type.

    The point isn't the styling. It's that **governance becomes the default instead of a discipline problem.** When the correct structure is generated automatically, every document ships with an ID, an owner, a classification, and a review cycle — because the author would have to work to leave them out. That's the difference between hoping people follow the standard and building a system where following it is the path of least resistance.

    The organization shown, **"Meridian Managed Services," is entirely fictional** — invented for these samples so nothing real is exposed. The brand, scenarios, and content are made up. What's real is the structure, the component design, and the information architecture.

## What's in these documents, and why I put it there

None of the pieces are decoration. Each one earns its place by solving a specific problem for whoever's reading:
...
Governed knowledge base articles are one half of the job. The other half is developer-facing documentation — user guides, installation guides, API references, tutorials — and that content lives in a different format for good reasons. [See the developer documentation →](developer-docs.md)
